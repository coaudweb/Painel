...
class TabelasApp {
    constructor() {
        ...
        this.paginationConfig = {
            video: {
                ...
                2: { start: 'M', end: 'Z', label: 'M-Z' }
            },
            audio: {
                1: { start: 'A', end: 'Z', label: 'A-Z' },
                2: { start: 'J', end: 'Z', label: 'J-Z' }
            }
        };
     ...
     */
    setupPagination() {
        // Paginação removida - todos os dados em uma página
    }
    ...
    addAlternatingCell(row, nomeEvento, observacao, tabId, groupCounter, index) {
        ...
        // Se não há observação, mostrar apenas o nome do evento em preto
        if (!observacao || observacao.trim() === '') {
            cell.textContent = nomeEvento;
            cell.style.color = '#333333'; // Preto para nome do evento
        } else {
            // Se há observação, iniciar com nome do evento em preto e marcar para alternância
            cell.textContent = nomeEvento;
            cell.style.color = '#333333'; // Preto para nome do evento
    ...
    startAlternatingCells(tabId) {
        ...
        alternatingCells.forEach(cell => {
            ...
            const interval = setInterval(() => {
                if (showingNome) {
                    cell.textContent = observacao;
                    cell.style.color = '#9c0606ff'; // Vermelho para observação
                    showingNome = false;
                } else {
                    cell.textContent = nomeEvento;
                    cell.style.color = '#333333'; // Preto para nome do evento
